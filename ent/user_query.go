...
func (uq *UserQuery) QueryCheckins() *CheckinQuery {
	query := &CheckinQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withCheckins: uq.withCheckins.Clone(),
...
func (uq *UserQuery) WithCheckins(opts ...func(*CheckinQuery)) *UserQuery {
	query := &CheckinQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withCheckins = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withCheckins; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Checkins = []*Checkin{}
		}
		query.withFKs = true
		query.Where(predicate.Checkin(func(s *sql.Selector) {
			s.Where(sql.InValues(user.CheckinsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_checkins
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_checkins" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_checkins" returned %v for node %v`, *fk, n.ID)
			}
